...
<body>
  ...
  <div id="leftside-navigation" class="nano">
    <ul class="nano-content">
      ...
      <li>
        <a href="../login/common_home.php.php"><i class="fa fa-power-off"></i><span>Log Out</span></a>
      </li>
    </ul>
  </div>
</aside>
...
<div align = "center" width = "700px">
	<fieldset>
		<?php //select availabe tasks
		$sql = mysqli_query($conn,"select distinct task_id,task_name,sub_id,company,task_desc from image natural join task where image_ans = ''"); 
		...
		<p>
			<div class = "login-box animated fadeInLeft">
				<div class = "article">
					<h2><?php
					echo '<br>';
					echo $row["task_name"];   //take task info
					$taskID = $row["task_id"];
					$subID = $row["sub_id"];
					echo $row["sub_id"];
					?></h2>
					<p> <?php echo "Company : ";echo $row["company"]; echo '<br>'; echo $row["task_desc"];?></p>
					<a href="do_image.php?sent=<?php echo $taskID;?>&sub=<?php echo $subID;?>">Select the Task</a> <!--navigate to answer -->
				</div>
			</div>
		</p>
		<?php

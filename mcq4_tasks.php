...
<fieldset>
	<?php //selecting available tasks
	$sql = mysqli_query($conn,"select distinct task_id,task_name,sub_id,company,task_desc from mcq4 natural join task where mcq4_ans = ''"); 
	...
	<p>
		<div class = "login-box animated fadeInLeft">
			...
			<?php
				echo '<br>';
				echo $row["task_name"];   //displaying task info
				$taskID = $row["task_id"];
				$subID = $row["sub_id"];
				echo $row["sub_id"];
			?></h2>
			<p> <?php echo "Company : ";echo $row["company"]; echo '<br>'; echo $row["task_desc"];?></p>
			<a href="do_mcq4.php?sent=<?php echo $taskID;?>&sub=<?php echo $subID;?>">Select the Task</a> 
			</div>
		</div>
	</p>
	<?php

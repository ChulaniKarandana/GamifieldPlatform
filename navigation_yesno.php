...
if ((isset($_POST['task'])) && (isset($_POST['points']))){
	...
	$query_sub = 'SELECT * FROM task WHERE task_id = "'.$var_task.'"';
	$result_sub = mysqli_query($conn,$query_sub);

	 while($row1 = mysqli_fetch_array($result_sub))
	 {
		 $rem = $row1['task_rem'];    //setting up values
		 $tot = $row1['task_total'];
		 $up = $rem - 1;
		 $sub = $tot - $up;
		 echo $up;
		 ...
		 if ($up >= 0){
			...
			//add points
			session_write_close();
			header("location:../../semester5project/adder_tasks/yesno.php"); //navigate to add questions
			exit;
		 }
		 else{

...
.sidebar {
  ...
  background: -moz-linear-gradient(top, rgba(206,220,231,0.02) 0%, rgba(204,218,229,0.02) 2%, rgba(89,106,114,1) 100%); /* FF3.6-15 */background: -webkit-linear-gradient(top, rgba(206,220,231,0.02) 0%,rgba(204,218,229,0.02) 2%,rgba(89,106,114,1) 100%); /* Chrome10-25,Safari5.1-6 */background: linear-gradient(to bottom, rgba(206,220,231,0.02) 0%,rgba(204,218,229,0.02) 2%,rgba(89,106,114,1) 100%); /* W3C, IE10+, FF16+, Chrome26+, Opera12+, Safari7+ */
...
<div class="my">
<h1 align = "center"><font color = "blue"> Gamifield Crowdsourcing Platform - Messages</font></h1>
</div>
	<form method = "post" action = "" align = "right">
		...
		<div class="login-box animated fadeInUp">
				<?php //show messages
				$id = $_SESSION['user_id'];
				$query_msg = 'SELECT *FROM sub_task_status natural join task WHERE add_id = "'.$id.'" and sub_notify = "no" and status = "done"';
				$result_query_msg = mysqli_query($conn,$query_msg); //select questions of the selected task

				while($row_msg = mysqli_fetch_array($result_query_msg))
				{
					?>
					<fieldset>
					<img src = "images/message.png" width = "50px" height = "50px" align = "left">
					<?php
					echo "<br>";
					$msg_task = $row_msg['task_name'];
					$msg_task_id = $row_msg['task_id'];
					$msg_sub_id = $row_msg['sub_id'];
					$sub_cat = $row_msg['sub_category'];
					echo "You have a completed ";
					echo $sub_cat;
					echo " sub-task under task: ";
					echo $msg_task;
					
				?>

				<input type="button" value="Noted" onclick="window.location.href='msg_marker.php?task=<?php echo $msg_task_id;?>&sub=<?php echo $msg_sub_id;?>'"/>
				</fieldset>
				<?php
				echo "<br>";
				}

...
<body>
  ...
  <div class="body"></div>
		...
		<div class="header">
			...
			<br>
		<form method = "post" action = "crowd_login.php">
			<div class="login">
...
if ((isset($_POST["username"])) && (isset($_POST["pass"]))){
	...
	$query="SELECT *FROM crowd_user WHERE crowd_user='".$new_username."'";   //query for username
	if ($is_query_run=mysql_query($query,$conn)){
		while($row=mysql_fetch_array($is_query_run,MYSQL_ASSOC)){
		
			if ($new_password == $row['crowd_pass']){
				$_SESSION['type'] = "crowd";              //assigning sessions to values to be remembered
				$_SESSION['username'] = $new_username;
				$_SESSION['user_id'] = $row['crowd_id'];
				header("location: ../crowd_tasks/crowd_dash.php");  //navigation for next page
			}
			else{ 
				echo "Error user";
			}

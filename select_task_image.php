...
<div class = "my">
	<h1 align = "center"><font color = "blue"> Gamifield Crowdsourcing Platform <br> Welcome to Image Labeling</font></h1>
</div>
<form method = "post" action = "navigation_image.php" align = "center">
	<div class="login">
		<div class = "login-box animated fadeInDown">
			<fieldset>
				<img src = "images/work.gif" width = "350px" height = "225px"><br><br>
					<label>Select the Task:</label>
					<?php 
						$company = $_SESSION['user_company'];
						$conn1 = new PDO('mysql:host = localhost;dbname=gamifield','root','chul@P292');  //creating PDO object
						$sql = 'SELECT task_id,task_name FROM task where company = "'.$company.'"';  //query the tasks owned by company
						$stmt = $conn1->prepare($sql);
						$stmt->execute();
						$users = $stmt -> fetchAll();
					?>
					<select name="task"><?php foreach($users as $row):?>
						<option value="<?=$row['task_id'];?>"><?=$row['task_name'];?></option>
						<?php endforeach; ?>
					</select>
					<br><br>
					<label>Points: </label><input type = "number" min = "0" name = "points" value = "points"/>
					<br><br><br>
